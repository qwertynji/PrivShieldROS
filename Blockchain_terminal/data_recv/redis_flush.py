...
#线程开关控制变量
thread_on = True
sm2_1 = 92006472779768011833073971106
...
# Redis连接函数
def Connect_redis():
    pool = redis.ConnectionPool(host='8.134.222.175', port=6379, deploy_contract=True,max_connections=10)
                ...
                if command == 'easy_register':
                    ...
                    try:
                        ...
                        if Recv_Data['username'] in keys:
                            ...
                            r.rpush(username, len_)
                            r.rpush(username, passwd)
                            back = register(token, accounts, len_, str(passwd))
                            if back != False:
